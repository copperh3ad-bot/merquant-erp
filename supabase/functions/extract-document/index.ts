...
const ANTHROPIC_TIMEOUT_MS = 240_000;         // 240s per attempt — large master_data XLSX with 8 sections can take 2-3 minutes on Haiku
const ANTHROPIC_MAX_TOKENS = 64_000;          // Haiku 4.5 native limit. master_data with 8 sections × ~250 rows needs ~60k. Smaller files only bill actual usage.
...
async function callAnthropicOnce(
  ...
  try {
    ...
    // Claude hit max_tokens before finishing the tool call. The output is
    // truncated mid-string/mid-array and the parsed JSON cannot be trusted —
    // trailing sections (e.g. carton, price_list, suppliers) come back empty
    // and silently apply as missing data. Surface this as a distinct kind.
...
async function callAnthropicChain(
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    if (result.kind === "truncated") {
      // The model billed for the full max_tokens output, so account the cost
      // before bailing. Don't escalate to the next model — a larger model
      // would also truncate at the same cap. The user must split the file.
      totalCost += computeCostUsd(result.model, result.usage);
...
Deno.serve(async (req) => {
  ...
  if (chain.kind === "exhausted") {
    ...
    if (lastFailure.kind === "truncated") {
      // Distinct from a generic LLM error: the call billed for the full
      // max_tokens output but cut off mid-tool-call. Trailing sections of
      // the JSON are missing and unsafe to apply (they'd silently look
      // empty rather than incomplete). Persist with EXTRACTION_LLM_TRUNCATED
      // so the user gets a clear "split the file" message rather than a
      // misleading partial-success state.
      await supabase.from("ai_extractions").insert({
        ...baseRow, model: lastModel, validation_status: "skipped",
        review_status: "rejected" as const,
        rejected_at: new Date().toISOString(),
