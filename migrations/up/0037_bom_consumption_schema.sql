...
-- UNIQUE constraints can only reference columns; use UNIQUE INDEX with COALESCE instead.
CREATE UNIQUE INDEX IF NOT EXISTS uniq_size_masters_cat_code_buyer
  ON size_masters (category, size_code, COALESCE(buyer_id, ''));
CREATE INDEX IF NOT EXISTS idx_size_masters_category ON size_masters(category, size_code);
...
-- UNIQUE constraints can only reference columns; use UNIQUE INDEX with COALESCE instead.
CREATE UNIQUE INDEX IF NOT EXISTS uniq_article_components_art_type_piece_order
  ON article_components (article_id, component_type, COALESCE(set_piece_name, ''), display_order);
CREATE INDEX IF NOT EXISTS idx_article_components_article ON article_components(article_id);
CREATE INDEX IF NOT EXISTS idx_article_components_type    ON article_components(component_type);
