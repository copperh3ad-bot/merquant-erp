...
function assessMilestoneRisk(
  ...
  const dueDate       = milestone.due_date ?? milestone.planned_date;
  ...
  if (milestone.completed_date || milestone.status === "completed" || milestone.status === "approved") {
    return {
      milestone,
      riskLevel: "on_track",
      daysRelative: 0,
...
async function processCalendar(
  ...
  const { data: po } = await supabase
    ...
    .select("po_number, buyer_name, delivery_date, season_id")
    .eq("id", poId)
    .maybeSingle();
  ...
  for (const risk of actionable) {
    ...
    const dueDate    = (risk.milestone.due_date ?? risk.milestone.planned_date) as string;
    ...
    try {
      ...
      const emailDraft = await draftBuyerEmail(anthropicKey, {
        ...
        seasonName:            calendar.season_name as string ?? null,
      });
