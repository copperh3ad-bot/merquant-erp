...
async function calculateBOM(
  ...
  // Fetch article to get product category
  ...
  const category = inferCategory(article?.category ?? article?.description ?? "");
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Run engine self-tests if requested
    if (run_tests) {
      const testResults = runEngineTests();
